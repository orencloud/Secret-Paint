...
def context3sculptbrush(context,**kwargs):
    ...
    if activeobj.type == "CURVES":
        ...
        for bb in brush_density:
            ...
            paint_modifier = _secret_paint_1731_paint_modifier(bpy.context.object)
            if paint_modifier:
                bb.curves_sculpt_settings.minimum_distance = _secret_paint_1731_modifier_value(
                    paint_modifier, "Socket_11", 0.1
                )
...
def secretpaint_function(self,*args,**kwargs):  
    ...
    or ActiveMode == "OBJECT" and N_Of_Selected == 0:
        ...
        if max(obj_for_dimensions.dimensions)>0:
            _secret_paint_1731_set_modifier_value(
                hair_modifier, "Input_68", (1 / (max(obj_for_dimensions.dimensions) ** 2)) * 2
            )
...
def _secret_paint_q_enable_selection_overlays(context):
    """Enable viewport overlays for the duration of a picker modal."""
    spaces = []
    seen_spaces = set()
    context_space = getattr(context, "space_data", None)
    if context_space is not None:
        spaces.append(context_space)
        seen_spaces.add(id(context_space))
    ...
    for area in getattr(screen, "areas", ()) if screen is not None else ():
        ...
        if space is not None and id(space) not in seen_spaces:
            spaces.append(space)
    ...
    for space in spaces:
        ...
        try:
            if not overlay.show_overlays:
                states.append((overlay, False))
                overlay.show_overlays = True
                for area in getattr(screen, "areas", ()) if screen is not None else ():
                    if getattr(area.spaces, "active", None) is space:
                        area.tag_redraw()
...
def _secret_paint_q_prompt_draw():
...
def _secret_paint_q_instance_owner_cache(context, system_cache):
    """Cache exact GN owners and a BVH of displayed instance bounds cubes."""
    ...
    try:
        ...
        paint_systems = {
            ...
            if _secret_paint_q_is_paint_system(obj)
        }
        bounds_modes = {}
        for pointer, system in paint_systems.items():
            ...
            bounds_modes[pointer] = (
                system_visible and
                getattr(system, "display_type", "SOLID") == "BOUNDS",
                brush_object if (
                    system_visible and
                    brush_object is not None and
        ...
        for instance in depsgraph.object_instances:
            ...
            parent = paint_systems.get(parent.as_pointer())
            if parent is None:
                continue
            ...
            owners.setdefault(key, parent)
            system_uses_bounds, bounded_brush = bounds_modes.get(
                parent.as_pointer(),
                (False, None),
            )
            if not system_uses_bounds and source != bounded_brush:
                continue
...
def _secret_paint_q_instance_system_from_hit(
    ...
    if not allow_cache_build:
        return None
    try:
        key = (source.as_pointer(), hit_matrix.copy().freeze())
    except (AttributeError, ReferenceError, TypeError, ValueError):
        return None
    owner = _secret_paint_q_instance_owner_cache(
        context,
        system_cache,
    ).get(key)
...
def _secret_paint_q_display_line_hit(
    ...
    if display_type == "BOUNDS":
        bounds_object = display_owner
        bounds_matrix = display_owner.matrix_world
        if display_owner == original_hit_object:
            bounds_matrix = hit_matrix
        vertices = list(bounds_object.bound_box)
        edges = (
            (0, 1), (1, 2), (2, 3), (3, 0),
            (4, 5), (5, 6), (6, 7), (7, 4),
            (0, 4), (1, 5), (2, 6), (3, 7),
...
def _secret_paint_q_bounds_system_under_cursor(
    ...
    bounds_bvh, bounds_face_owners, bounded_instance_systems = cache.get(
    ...
    if bounds_bvh is not None:
        try:
            ...
            if (face_index is not None and
                    0 <= face_index < len(bounds_face_owners)):
                nearest = bounds_face_owners[face_index]
                nearest_distance = distance
        except (AttributeError, ReferenceError, RuntimeError, TypeError, ValueError):
            pass
...
def _secret_paint_q_paint_system_from_ray(
    ...
    try:
        ...
        for _depth in range(32):
            ...
            if bool(original_hit_object.get(
            ...
            display_type = getattr(display_owner, "display_type", "SOLID")
            if ray_state is not None:
                ray_state.update(
                    hit=True,
            ...
            if (paint_system is not None and select_bounds_system_geometry and
    ...
    if view_area is not None and region is not None and coord is not None:
        ...
        try:
            ...
            needs_bounds_index = (
                ...
                exact_instance_targeting and
                not _secret_paint_q_is_paint_system(fast_candidate)
            )
            if needs_bounds_index:
                bounds_candidate, bounds_distance = (
                    _secret_paint_q_bounds_system_under_cursor(
                        context,
                        ray_origin,
                        ray_direction,
                        system_cache,
                ...
                ray_location = ray_state.get("location")
                ray_distance = float("inf")
                if ray_location is not None:
                    ray_distance = max(
                        0.0,
                        (Vector(ray_location) - Vector(ray_origin)).dot(
                            Vector(ray_direction).normalized()
                        ),
                    )
                if (bounds_candidate is not None and
                        bounds_distance <= ray_distance + 1.0e-4):
        ...
        requires_native_pick = (
            not ray_state.get("display_filtered", False) and
            not fast_candidate_is_system
            and (
                fast_candidate is None
                or display_type in {"WIRE", "BOUNDS"}
                or fast_candidate.type in {"EMPTY", "FONT", "META", "SURFACE"}
            )
        )
        ...
        moved_for_native = (
            ...
            or abs(coord[0] - last_native_coord[0]) >= 4
            or abs(coord[1] - last_native_coord[1]) >= 4
        )
        native_pick_due = (
            requires_native_pick
            and exact_instance_targeting
            and moved_for_native
            and now - last_native_time >= 0.12
        )
        if native_pick_due:
...
def _secret_paint_q_existing_system(terrain, brush_object):
    """Return an existing system on terrain that already uses brush_object."""
...
def _secret_paint_q_cached_systems_for_brush(brush_object, cache=None):
    ...
    key = getattr(brush_object, "name", None)
    if key not in source_cache:
        source_cache[key] = _secret_paint_q_systems_for_brush(brush_object)
...
def _secret_paint_q_create_system(self, context, terrain, brush_object):
...
def _secret_paint_q_begin_plant_selection(
    ...
    self._q_draw_handler = bpy.types.SpaceView3D.draw_handler_add(
        _secret_paint_q_prompt_draw, (), 'WINDOW', 'POST_PIXEL'
    )
    ...
    def invoke(self, context, event):
        ...
        if (
            ...
            if self._q_candidate is not None:
                ...
                context.view_layer.objects.active = self._q_candidate
            context.window_manager.modal_handler_add(self)
            context.area.tag_redraw()
            return {'RUNNING_MODAL'}
        ...
        if hasattr(self, "_q_terrain"):
            ...
            if event.type == 'MOUSEMOVE' or (
                ...
                if context.area and context.area.type == 'VIEW_3D':
                    ...
                    preview_state = getattr(self, "_q_mask_preview", None)
                    ...
                    if preview_state:
                        preview_system = preview_state.get("system")
                        preview_terrain = getattr(
                            preview_system,
                            "parent",
                            None,
                        )
                        preview_location = (
                            _secret_paint_q_preview_mask_location(
                                context,
                                event,
                                preview_system,
                        ...
                        if (picked in {None, preview_terrain} and
                                preview_location is not None):
                            picked = preview_system
                    self._q_candidate = picked
...
class paint_change_terrain(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        self._terrain_draw_handler = bpy.types.SpaceView3D.draw_handler_add(
            _secret_paint_q_prompt_draw, (), 'WINDOW', 'POST_PIXEL'
        )
        _secret_paint_q_prompt = True
        _secret_paint_q_begin_selection_mode(
...
def _secret_paint_1731_build_layout_model(context, obj):
    ...
    for biome in sorted(grouped, key=lambda value: str(value)):
        ...
        biomes.append({
            ...
            "render_icon": "RESTRICT_RENDER_ON" if any(row["render_alert"] for row in rows) else "RESTRICT_RENDER_OFF",
...
class legacy_panel_keyboard_reorder(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        step = int((event.mouse_y - self._start_mouse_y) / 18)
        if step == self._last_step:
            return {'RUNNING_MODAL'}
        active_object = bpy.data.objects.get(self._object_name)
        if active_object is None:
            return {'CANCELLED'}
        _secret_paint_1731_reorder_selected_panel_rows(
            context,
            active_object,
            direction=-1 if step > self._last_step else 1,
        )
...
def _secret_paint_1731_shortcut_label(operator_id, fallback="", keymap_name="Sculpt Curves"):
    """Format the currently customized shortcut for a panel button."""
    try:
        wm = bpy.context.window_manager
        keyconfigs = [wm.keyconfigs.user, wm.keyconfigs.addon]
        for keyconfig in keyconfigs:
            for keymap in keyconfig.keymaps:
                if keymap_name and keymap.name != keymap_name:
                    continue
                for keymap_item in keymap.keymap_items:
                    if keymap_item.idname != operator_id or not keymap_item.active:
                        continue
                    parts = []
                    if keymap_item.ctrl:
                        parts.append("Ctrl")
                    ...
                    if keymap_item.oskey:
                        parts.append("Cmd")
                    if keymap_item.type and keymap_item.type != "NONE":
                        parts.append(keymap_item.type.replace("NUMPAD_", "Num ").title())
                    if parts:
                        return "+".join(parts)
    except Exception:
        pass
...
def _secret_paint_1731_compact_panel_draw(self, context):
    ...
    if _secret_paint_q_selection_mode in {"PLANT", "TERRAIN"}:
        ...
        plant_shortcut = _secret_paint_1731_shortcut_label("secret.paint", "Q")
        terrain_shortcut = _secret_paint_1731_shortcut_label(
            "secret.paint_change_terrain", "Ctrl+Q"
        )
        density_shortcut = _secret_paint_1731_shortcut_label(
            "secret.brush_density_while_painting", "D"
        )
        paint_row.operator(
            "secret.paint",
            icon='BRUSH_DATA',
    ...
    for biome in _secret_paint_1731_layout_model(context, context.object):
        biome_box = layout.box()
        header, panel = biome_box.panel(str(biome["bgroup"]), default_closed=False)
        draw_biome(biome, header)
        if panel:
            rows = panel.column(align=True)
...
def register():
    ...
    km = kc.new(name="User Interface")
    kmi = km.keymap_items.new("secret.panel_keyboard_reorder", "G", "PRESS")
    addon_keymaps.append((km, kmi))
    kmi = km.keymap_items.new("secret.panel_keyboard_delete", "X", "PRESS")
    addon_keymaps.append((km, kmi))
    ...
    kmi = km.keymap_items.new("secret.paint", "Q", "PRESS")
    addon_keymaps.append((km, kmi))
    kmi = km.keymap_items.new("secret.paint_change_terrain", "Q", "PRESS", ctrl=True)
    addon_keymaps.append((km, kmi))
    ...
    km = kc.get("Object Mode")
    if not km:
        km = kc.new("Object Mode", space_type="VIEW_3D", region_type="WINDOW")
    ...
    if not km:
        km = kc.new("Curve")
    kmi = km.keymap_items.new("secret.curveseparate", "Q", "PRESS", ctrl=True, alt=True)
    addon_keymaps.append((km, kmi))
